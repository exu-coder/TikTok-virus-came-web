...
def get_finger_frame(image, landmarks):
    ...
    corners = [
        (int(center_x - dx * cos_a - dy * sin_a), int(center_y - dx * sin_a + dy * cos_a)),
        (int(center_x + dx * cos_a - dy * sin_a), int(center_y + dx * sin_a + dy * cos_a)),
        (int(center_x + dx * cos_a + dy * sin_a), int(center_y + dx * sin_a - dy * cos_a)),
        (int(center_x - dx * cos_a + dy * sin_a), int(center_y - dx * sin_a - dy * cos_a)),
    ]
...
def apply_neon_effect(frame):
    ...
    return cv2.add(darkened, glow)

def apply_invert_effect(frame):
    inverted = cv2.bitwise_not(frame)
    blue_tint = np.zeros_like(frame)
    blue_tint[:, :] = (100, 50, 0)
    return cv2.addWeighted(inverted, 0.8, blue_tint, 0.2, 0)
...
def warp_perspective_to_portal(frame, corners, portal_w, portal_h):
    dst_pts = np.array([[0, 0], [portal_w, 0], [portal_w, portal_h], [0, portal_h]], np.float32)
...
def main():
    ...
    time_val = 0
    print("Controls: SPACE = Change filter | Q = Quit")
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                if portal_w > 50 and portal_h > 50 and portal_w < w and portal_h < h:
                    ...
                    label_x, label_y = corners[0][0], corners[0][1] - 10
                    cv2.rectangle(output, (label_x, label_y - label_size[1] - 5),
                                  (label_x + label_size[0], label_y + 5), (0, 0, 0), -1)
